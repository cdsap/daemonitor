...
#   ./cored/scripts/dual-run-linux.sh --docker
set -euo pipefail
...
if [[ "$USE_DOCKER" -eq 1 ]]; then
  ...
  docker run --rm \
    ...
    bash -lc '/work/cored/scripts/dual-run-linux.sh --inside-docker'
  exit $?
fi
...
cored/bin/daemonitor-cored -socket "$SOCK" -db "$DB" -interval 500ms -retention 1h &
CORE_PID=$!
cleanup() {
...
cored/bin/daemonitor-corectl -socket "$SOCK" builds | head -20 || true
