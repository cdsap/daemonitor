...
func (m Model) renderHeader(width int) string {
	...
	line2 := truncateRunes(fmt.Sprintf("%d processes   %s RSS   %s   Refresh %s",
		len(m.processes), formatMemMB(totalRSS), coreStatus, refresh), width)
...
func (m Model) renderTable(width int) string {
	...
	var b strings.Builder
	b.WriteString(headerStyle.Render(truncateRunes(formatHeader(cols, width), width)))
	b.WriteByte('\n')
...
func (m Model) renderFooter(width int) string {
	compact := "↑/↓ or j/k select   Home/g End/G   ? help   q quit"
	if m.showHelp {
		compact = "Navigation: ↑/↓ j/k move · Home/g first · End/G last\n" +
			"? close help · q quit · more keys in later slices"
	}
	return footerStyle.Render(clampBlock(compact, width))
}
...
func formatHeader(cols columnSet, width int) string {
	parts := make([]string, 0, len(cols.ids))
	for _, id := range cols.ids {
		parts = append(parts, colHeader(id, cols.projectWidth))
	}
	return truncateRunes(strings.Join(parts, "  "), width)
}

func colHeader(id columnID, projectWidth int) string {
	switch id {
	case colType:
	...
	case colProject:
		return padRight("PROJECT", projectWidth)
	default:
		return ""
	}
}
